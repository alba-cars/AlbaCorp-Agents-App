...
part 'edit_pocket_listing_state.dart';
part 'edit_pocket_listing_cubit.freezed.dart';

@injectable
class EditPocketListingCubit extends Cubit<EditPocketListingState> {
  EditPocketListingCubit(this._leadRepo, this._listingsRepo, this._explorerRepo,
      @factoryParam PropertyCardDetailsModel? propertyCard)
      : super(EditPocketListingState(
            propertyCard: propertyCard,
            rawValues: propertyCard?.toJson() ?? {})) {
    getPropertyTypes();
  }
  ...
  Future<void> updatePocketListing(
      {required Map<String, dynamic> values}) async {
    emit(state.copyWith(updatePocketListingStatus: AppStatus.loading));
    final result = await _explorerRepo.updatePropertyCard(
        values: values, propertyCardId: state.propertyCard!.id);
    switch (result) {
      case (Success s):
        emit(state.copyWith(updatePocketListingStatus: AppStatus.success));
        break;
      case (Error e):
        emit(state.copyWith(
            updatePocketListingStatus: AppStatus.failure,
            updatePocketListingError: e.exception));
        break;
    }
  }
  ...
  Future<void> onNextPressed(BuildContext context,
    ...
    switch (state.currentTab) {
      case 0:
        ...
        return;
      case 1:
        await updatePocketListing(values: state.values);
        if (state.updatePocketListingStatus == AppStatus.success) {
          if (context.mounted) {
            showSnackbar(context, 'Successfully updated pocket listing',
                SnackBarType.success);
            context.pop(true);
          }
        } else if (state.updatePocketListingStatus == AppStatus.failure) {
          if (context.mounted) {
            showSnackbar(context, state.updatePocketListingError ?? '',
                SnackBarType.success);
          }
        }
        return;
